...
- Uploaded `1.0.4_` to APPID `tt02d6746b9cb2fc0e10`.
...
- Platform version page shows debug package `1.0.4_` and status `部署完成`.
...
Next step:
- Relaunch the debug gameplay from phone/live companion so the running session picks up `1.0.4_`, then visually confirm the round avatars are readable but not blocking soldier heads.
...
What changed:
- Opened the local web preview with the deployed Douyin Cloud URL:
  `http://127.0.0.1:8080/index.html?v=1781420800000&liveCloudUrl=https%3A%2F%2F1m3j5q7o3dezm-env-cuABsk2rKR.service.douyincloud.run`
- Confirmed the preview client connected to the deployed cloud service and initialized the live bridge.
- After the preview synced at cloud event seq `6`, pushed one new platform self-test like event from the Open Platform self-test tool.
- Confirmed the deployed cloud queue advanced to seq `7` and the local preview client consumed that event.
- Rechecked the Open Platform version page: debug package `1.0.0_` is still `部署完成`, cloud start is enabled, resolution is `1080P`, display ratio is `9:16`.
- Rechecked the `开播测试账号` page through the platform menu: one test member is present and shows status `已生效`.
...
Commands run:
- `GET https://1m3j5q7o3dezm-env-cuABsk2rKR.service.douyincloud.run/api/health`
- `GET https://1m3j5q7o3dezm-env-cuABsk2rKR.service.douyincloud.run/api/live/events?after=6`
- Chrome platform self-test push on `https://developer.open-douyin.com/sonic/tt02d6746b9cb2fc0e10/develop/diagnose_tool?subTab=1&tab=openAPI`
- Browser preview on `http://127.0.0.1:8080/index.html?v=1781420800000&liveCloudUrl=...`

Verification:
- Preview console logged `云服务已连接: https://1m3j5q7o3dezm-env-cuABsk2rKR.service.douyincloud.run`.
- Preview console logged `Live interaction bridge ready: window.__JZTW_LIVE__.comment/like/gift`.
- Preview console logged `Live cloud synced at seq 6`.
- After pushing a new platform self-test like, cloud `GET /api/live/events?after=6` returned seq `7`, `msgType: live_like`, `count: 10`, `source: douyin-platform, internal-callback`.
- Preview console then logged `viewer join 7650439220800394267 x1 11 Heavy Pistol active=1 reserve=0` and `点赞出兵: 11x1`, proving the platform self-test -> Douyin Cloud -> `/api/live/events` -> local gameplay client path.

Risks / notes:
- This still is not final official live-room validation because the event is generated by the platform self-test tool, not a real live/debug room launch.
- The formal next gate is in the live/companion side: use the active test member account, set it private if required, start the debug package from Live Companion or the Douyin live room interaction panel by searching the debug package, then send real comment, like, and gift events.
- Current cloud diagnostics still show the `/start_game` entry as `reason: auto:/live_data_callback`; a real official launch should produce a true platform start context and real-room callbacks.

Next step:
- Launch `testlevel002-调试包` with the active test member account from the official live/debug entry and verify that diagnostics show real room context, then confirm real comment/like/gift events spawn soldiers in the game client.
