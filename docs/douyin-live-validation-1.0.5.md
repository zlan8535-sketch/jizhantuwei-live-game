# Douyin Live Validation Sheet - 1.0.5

APPID: `tt02d6746b9cb2fc0e10`
Debug package: `1.0.5_`
...
Use this sheet to validate the deployed `1.0.5_` package from the real phone/live companion flow. Local preview and Open Platform self-test are useful smoke tests, but this sheet is only passed when the real live/debug entry displays the expected gameplay feedback.

## Preflight

- Open the debug package from phone/live companion, not from an already-running old session.
- Confirm the package shown in Open Platform is `1.0.5_` and status is deployed.
...
- Normal viewer avatar size should be readable on the phone live view, currently `18px`.
- Giant viewer avatar size should be readable on the phone live view, currently `22px`.
...
- The gameplay running from `1.0.5_` consumes the callbacks without local query parameters.
